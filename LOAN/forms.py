...
class customerKYCform(forms.ModelForm):
    def __init__(self, request, *args, **kwargs):
        ...
        self.fields['BankName'] = forms.ModelChoiceField(
            queryset=BankName.objects.all().order_by("BankName_value"),label="Bank Name"
        )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "Custimage": forms.ClearableFileInput(
                attrs={"type": "file", "id": "image-preview__image9","accept":"*" ,'class': 'camera-input'}
            ),
            "Member_Aadhar_Card_front": forms.ClearableFileInput(
                attrs={"type": "file", "id": "image-preview__image1","accept":"*", 'class': 'custom-file-input'}
            ),
            "Member_Aadhar_Card_back": forms.ClearableFileInput(
                attrs={"type": "file", "id": "image-preview__image2","accept":"*", 'class': 'custom-file-input'}
            ),
            "Member_Voter_Card_front": forms.ClearableFileInput(
                attrs={
                    "type": "file",
                    "id": "image-preview__image3","accept":"*",
            ...
            "Member_Voter_Card_back": forms.ClearableFileInput(
                attrs={
                    "type": "file",
                    "id": "image-preview__image4","accept":"*",
                    "name": "voter-card-backSide",
                }
            ),
            "Co_Insurer_Aadhaar_front": forms.ClearableFileInput(
                attrs={"type": "file", "id": "image-preview__image5","accept":"*", "class": "custom-file-input"}
            ),
            "Co_Insurer_Aadhaar_back": forms.ClearableFileInput(
                attrs={"type": "file", "id": "image-preview__image6","accept":"*", "class": "custom-file-input"}
            ),
            "Member_Bank_Details": forms.ClearableFileInput(
                attrs={"type": "file", "id": "image-preview__image7","accept":"*", "class": "custom-file-input"}
            ),
            "Member_Relationship_Proof": forms.ClearableFileInput(
                attrs={"type": "file", "id": "image-preview__image8","accept":"*", "class": "custom-file-input"}
            ),
...
class userForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['group'] = forms.ModelChoiceField(
            queryset=Group.objects.all(),label="Group Name" 
        )
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        try:
            ...
            if groups.group.name == "Branch Manager":
                ...
                choices=[(o.userName.id, str(f"{o.userName.first_name.title()} {o.userName.username}")) for o in userWithRole.objects.filter(branchName__id = managerBranch.branchName.id).exclude(Q(user_role__id = 2) | Q(user_role__id = 1))]
                ,label="Agent ID" 
            )
            else:
                self.fields['branch_Name'] = forms.ChoiceField(
                choices=[(o.id, str(f"{o.branch_Name}")) for o in branch.objects.all()]
                ,label="Branch Name"
                )
                self.fields["agents_Name"] = forms.ChoiceField(
                choices=[(o.userName.id, str(f"{o.userName.first_name.title()} {o.userName.username}")) for o in userWithRole.objects.all().exclude(Q(user_role__id = 2) | Q(user_role__id = 1))]
                ,label="Agent ID" 
                )
