    ...
    if request.method == "POST": 
        ...
        if Branch_Name:
            ...
            for agent in agents:
                ...
                agentCenterIdCount = center.objects.filter(branch_Name__id = Branch_Name,agents_Name__id = agent.agents_Name.id,center_Id=agent.center_Id)
...
def reverseEMI(request, TransactionId, loanId):
    ...
    for emi in reverse_emi:
        if str(emi.PaymentDate.date() + timedelta(days=1)) == str(
            ...
            auditData.save()
            emi.emiStatus = emiStatus.objects.get(id=3)
            emi.save()
    ...
    if request.method == "POST":
        ...
        if Date and Branch:
            ...
            ProcessCharge = disbursement_customer_kyc.objects.filter(
                BranchName__branch_Name__id=Branch,
...
def loanApplication(request):
    ...
    if request.method == "POST":
        ...
        try:
            if Branch_Name and applicationDate:
            ...
            elif Branch_Name and Center_Id and applicationDate:
                customer_data = customerKYC.objects.filter(
                    BranchName__branchName__id=Branch_Name,
...
def Collection_demand(request):
    ...
    if request.method == "POST":
        ...
        try:
            if Branch_Name and Center_Id:
                customer_data = disbursement_customer_kyc.objects.filter(
                    BranchName__branchName__id=Branch_Name, CenterId__id=Center_Id
                )
            elif Branch_Name and CSO_Name:
                customer_data = disbursement_customer_kyc.objects.filter(
                    BranchName__branchName__id=Branch_Name,
                    BranchName__userName__id=CSO_Name,
                )
            elif Demand_Date:
                customer_data = disbursement_customer_kyc.objects.filter(
                    Timestamp__date=Demand_Date
                )
            for data in customer_data:
                PendingEmi = loanEMI.objects.filter(loanId__loanId=data.loandata.loanId,emiStatus__id=2).first()
                notPaidEmi = loanEMI.objects.filter(loanId__loanId=data.loandata.loanId,emiStatus__id=3)
                for notPaidEmiAmount in notPaidEmi:
                    amount += notPaidEmiAmount.installmentAmount
                amount += PendingEmi.installmentAmount
        except:
            customer_data = None
        context = {"Region": Region, "customer_data": customer_data, "amount": amount,
            'role':role,
            'managerBranch':managerBranch}
    else:
        context = {
...
def Loan_Details(request):
    ...
    if request.method == "POST":
        ...
        if branchID and centerID:
            customer_data = disbursement_customer_kyc.objects.filter(
                    BranchName__branch_Name__id = branchID, CenterId__id = centerID
                )
...
def generate_pdf(request):
    ...
    if request.method == "POST":
        ...
        userData = disbursement_customer_kyc.objects.filter(
            ...
            BranchName__branch_Name__id=int(Branch_Name),
            Disbursement_Payment_Mode__id=2,
        )
        depositeAmounts = BankTransactions.objects.filter(
...
def dataUpdate(request):
    data = loanEMI.objects.filter(loanId__loanId = 1051)
    data.delete()
    HttpResponse("its done kailash")