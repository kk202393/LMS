...
def collection_details(request):
    ...
    if request.method == "POST":
        ...
        customerEmiData = loanEMI.objects.filter(
            center__id=Center_Id, PaymentDate = disbursementDate if disbursementDate  else str(date.today()), emiStatus__id=2
        )
...
def branchSummaryReport(request):
    ...
    if request.method == "POST": 
        ...
        if Branch_Name:
            ...
            for agent in agents:
                agentCenterId = loanEMI.objects.filter(center__branch_Name__id = Branch_Name,center__agents_Name__id =  agent.agents_Name.id, installmentDate= branchSummaryDate).exclude(emiStatus__id=2)
                agentCenterIdCount = loanEMI.objects.filter(center__agents_Name__id = agent.agents_Name.id,installmentDate= branchSummaryDate).exclude(emiStatus__id=2).values('center__center_Id').annotate(count=Count('center__center_Id'))
                agentSubmitEmaiData = loanEMI.objects.filter(center__center_Id = agent.center_Id,center__branch_Name__id = Branch_Name , PaymentDate= branchSummaryDate,center__agents_Name__id =  agent.agents_Name.id , emiStatus__id=2)
    ...
    context = {
        ...
        "agentSubmitEmaiData":agentSubmitEmaiData,
        "agents":agents,
...
def disburseCustomerDetails(request,id):
    ...
    for customerDetails in disburseCustDetails:
        try:
            distID = Dist.objects.get(Dist_Name=customerDetails.District)
        except  Dist.DoesNotExist:
            distID = Dist.objects.get(id=customerDetails.District)
        dist = Dist.objects.get(id=distID.id)
        confirmdist = Dist.objects.get(id=distID.id)
...
def CreateNewBranch(request):
    ...
    if request.method == "POST":
        haed_office_name = request.POST["haed_office_name"]
        circle_name = request.POST["circle_name"]
        Zone = request.POST["zone"]
        Region = request.POST["region"]
        branch_Name = request.POST["branch_Name"]
        createNewBranch = branch(
            haed_office_name = haed_office.objects.get(id=haed_office_name),
            circle_name = circle.objects.get(id=circle_name),
            zone = zone.objects.get(id=Zone),
            region = region.objects.get(id=Region),
            branch_Name = branch_Name
        )
        createNewBranch.save()
        messages.success(request, "Branch is Assigned successfully")
        # return render(request, "successRenderScreen.html")
        return redirect('successRenderScreen')
    else:
        form = branchform()
        return render(request,"createNewBranch.html",{"form": form,},)
...
def reverseEMI(request, TransactionId, loanId):
    ...
    for emi in reverse_emi:
        if str(emi.PaymentDate.date() + timedelta(days=1)) == str(
            ...
            auditData.save()
            emi.emiStatus = emiStatus.objects.get(id=1)
            emi.emiSubmittedBy = None
            emi.EmiPaymentMode = None
            emi.TransactionId = None
            emi.PaymentDate = None
            emi.save()
        ...
        try:
            if loanID:
                ...
                paidEmi = loanEMI.objects.filter(
                    loanId__loanId=loanReverseEmi.loandata.loanId, emiStatus__id=2
                )
                ...
                if max.get("TransactionId__max") != None:
                    reverseEMI = loanEMI.objects.filter(
                        ...
                        loanId__loanId=loanReverseEmi.loandata.loanId,
                    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if openingBalanceAmount:
                ...
                Account_TransactionID =  Account_Transaction.objects.aggregate(max=Max("id"))["max"]
                openingBalances = Account_Transaction.objects.get(id = Account_TransactionID)
                openingBalance = openingBalances.AccountBalance
        except:
            ClosingBalance = 0
        if Date and Branch:
            lastDayCloseDate = Account_Transaction.objects.aggregate(max=Max("TransDate"))["max"].date() + timedelta(days=int(2)) if Account_Transaction.objects.aggregate(max=Max("TransDate"))["max"] else datetime.strptime(str(datetime.today().date()), "%Y-%m-%d")  
            start = datetime.strptime(str(lastDayCloseDate), "%Y-%m-%d") if Account_Transaction.objects.aggregate(max=Max("TransDate"))["max"] else datetime.strptime(str(datetime.today().date()), "%Y-%m-%d")
            userInputDate = datetime.strptime(str(Date), "%Y-%m-%d")
            end = datetime.strptime(str(datetime.today().date()), "%Y-%m-%d")
            date_generated = [start + timedelta(days=x) for x in range(0, (end-start).days)]
            if userInputDate != start:
                if start < end:
                    messages.success(
            ...
            EmiData = loanEMI.objects.filter(
                PaymentDate=Date, center__branch_Name__id=Branch, emiStatus__id=2
            )
            ProcessCharge = disbursement_customer_kyc.objects.filter(
                BranchName__branchName__id=int(Branch),
                Disbursement_Date=Date,
                Disbursement_Payment_Mode__id=1,
            )
        #     userData = disbursement_customer_kyc.objects.filter(
...
def Agentbranch(request):
    if request.method == "GET":
        BranchID = request.GET.get("BranchID")
        BranchAgents = userWithRole.objects.filter(branchName__id = BranchID).exclude(Q(user_role__id=1) | Q(user_role__id=2))
    return render(request, "dropdown_list.html", {"branch_agents": BranchAgents})
...
def Preclosure_Amount(request):
    ...
    LPC = 0,
    form_status = 0
    form_status_name = None
    pre_closure_loanID = None
    error = None
    if request.method == "GET":
        cust_Id = request.GET.get("cust_Id")
        LoanIDs = customerKYC.objects.filter(
            loandata__customerId=cust_Id, status__id=4
        )
        
        try:
            totalLoanAmount = customerKYC.objects.get(
            loandata__customerId=cust_Id, status__id=2
            )
            for LoanID in LoanIDs:
                form_status = LoanID.status.id
                pre_closure_loanID = LoanID.loandata.loanId
                form_status_name = LoanID.status.form_STATUS_value 
            ...
            data = {
                ...
                "pre_closure_loanID" : pre_closure_loanID,
                "amount": totalLoanAmount.LoanAmount,
                "LPF":totalLoanAmount.Product.loanProcessingFee,
                "LPC":totalLoanAmount.Product.loanProcessingCharge,
            }
            return JsonResponse(data)
        except customerKYC.DoesNotExist:
            error = "Please first Approved customer KYC"
            data = {
                "error": error,
            }
            return JsonResponse(data)
...
def csoName(request):
    if request.method == "GET":
        ...
        cso_Name = userWithRole.objects.filter(branchName__id=branch).exclude(user_role__id =1)
    ...
    if request.method == "POST":
        ...
        try:
            if Branch_Name and fromDate and toDate:
                ...
                ).exclude(Q(status__id =3) | Q(status__id=4))
                ...
                customer_data =  customerKYC.objects.filter (BranchName__branchName__id=Branch_Name, CenterId__id=Center_Id).exclude(Q(status__id =3) | Q(status__id=4))
            elif ApplicationNo:
                customer_data = customerKYC.objects.filter(loandata__Application_No=ApplicationNo).exclude(Q(status__id =3) | Q(status__id=4)) 
        except:
            customer_data = None
        context = {
            "Region": Region,
...
def cutomer_data(request, id):  # customer data for branch manager
    ...
    status_data = (
        ...
        .exclude(Q(id=1) | Q(id=4)| Q(id=6)| Q(id=5))
    )
    user = request.user
    pre_closure_Amount = 0
    for data in custoner_data:
        LoanIDs = customerKYC.objects.filter(
            loandata__customerId=data.loandata.customerId, status__id=4
        )
        for LoanID in LoanIDs:
            # amount = amount +  LoanID.LoanAmount
            no_EMI_Pay = loanEMI.objects.filter(
            ...
            if not no_EMI_Pay:
                pre_closure_Amount = pre_closure_Amount + LoanID.LoanAmount
            elif not preclosureAmounts:
                messages.success(
                request, "This user has already Loan With No EMI Pay. Please Pay Atleast one EMI to continue this loan request"
                )
                return redirect('successRenderScreen')
            else:
                pre_closure_Amount = (
    ...
    if request.method == "POST":
        ...
        if Status == 2:
            ...
            try:
                ...
                rejected_appilaction.status.id == 3
                messages.success(request, f"This form is rejected please apply another Loan ${loan_data.loanId}")
                # return render(request, "successRenderScreen.html")
                return redirect('successRenderScreen')
            except rejected_customer_kyc.DoesNotExist:
                inst = customerKYC.objects.get(loandata__loanId=loan_data.loanId)
                inst.status = form_STATUS.objects.get(id=2)
                inst.save()
            try:
    ...
    if request.method == "POST":
        ...
        try:
            if Branch_Name and not Center_Id  and not CSO_Name:
                loanIds = loanEMI.objects.filter(center__branch_Name__id = Branch_Name,installmentDate = Demand_Date).exclude(emiStatus__id = 2)
                for id in loanIds:
                ...
                loanIds = loanEMI.objects.filter(center__branch_Name__id = Branch_Name,center__id = Center_Id).exclude(emiStatus__id = 2)
                for id in loanIds:
                    userLoanIdList.append(id.loanId.loanId)
                customer_data = disbursement_customer_kyc.objects.filter(
                    loandata__loanId__in = userLoanIdList
                ...
                loanIds = loanEMI.objects.filter(center__branch_Name__id = Branch_Name,center__agents_Name__id = CSO_Name,installmentDate = Demand_Date).exclude(emiStatus__id = 2)
                for id in loanIds:
                    userLoanIdList.append(id.loanId.loanId)
                customer_data = disbursement_customer_kyc.objects.filter(
                    loandata__loanId__in = userLoanIdList
...
def Loan_Summary_Report(request):
    ...
    if request.method == "POST":
        ...
        if Branch_Name and applicationFromDate and applicationToDate:
            customer_data = disbursement_customer_kyc.objects.filter(
                    BranchName__branchName__id=Branch_Name,status__id = 4,
                    Disbursement_Date__range=(applicationFromDate, applicationToDate)
                ).exclude(Q(status__id = 5)|Q(status__id = 6))
        # except:
        #     customer_data = None
        context = {
            "Region": Region,
    ...
    return render(request, "Loan_Summary_Report.html", context)
    ...
    if request.method == "POST":
        ...
        try:
            if Branch_Name and applicationFromDate and applicationToDate:
                ...
                customer_data = disbursement_customer_kyc.objects.filter(
                    BranchName__branchName__id=Branch_Name,status__id = 4, CenterId__id=Center_Id
                ).exclude(Q(status__id = 5)|Q(status__id = 6))
            elif ApplicationNo:
                customer_data = disbursement_customer_kyc.objects.filter(
                    loandata__Application_No=ApplicationNo,status__id = 4,
                ).exclude(Q(status__id = 5)|Q(status__id = 6))
        except:
            customer_data = None
        context = {
    ...
    if not loanEMI.objects.filter(loanId=customer_data.loandata):
        for v, x, y, z, a in array_data:
            obj = loanEMI(
                ...
                loanMode = Loan_mode_data.objects.get(id = customer_data.Product.loanMode.id)  ,
                installmentDate=v,
                principleAmount=y,
                interestAmoun=x,
...
def loan_disbursement(request, loanId):
    try:
        ...
        if request.method == "POST":
            ...
            Loan_Mode_Name = request.POST.get("Loan_Mode_Name")
            pre_closure_loanID = request.POST.get("pre_closure_loanID")
            cust_data = disbursement_customer_kyc(
                ...
                District= Dist.objects.get(id = customer_data.District).Dist_Name,
                ...
                confirmDistrict=Dist.objects.get(id = customer_data.confirmDistrict).Dist_Name ,
            ...
            inst.status = form_STATUS.objects.get(id=4)
            inst.save()
            cust_data.save()
            try:
                if int(Loan_Mode_Name) == 4:
                    preCloseCustomerEMIs = loanEMI.objects.filter(loanId__loanId = pre_closure_loanID).exclude(Q(emiStatus__id = 2)|Q(emiStatus__id = 4))
                    preCloseDisbursementCustomerKyc = disbursement_customer_kyc.objects.get(loandata__loanId = pre_closure_loanID)
                    preCloseDisbursementCustomerKyc.status = form_STATUS.objects.get(id = 5)
                    preCloseDisbursementCustomerKyc.save()
                    for preCloseCustomerEMI in preCloseCustomerEMIs:
                        Transaction_Id = (
                        1001
                        if loanEMI.objects.filter(loanId__loanId=pre_closure_loanID, emiStatus__id=2).count()
                        == 0
                        else loanEMI.objects.filter(
                        loanId__loanId=pre_closure_loanID, emiStatus__id=2
                        ).aggregate(max=Max("TransactionId"))["max"]
                        + 1
                        )
                        preCloseCustomerEMI.emiStatus = emiStatus.objects.get(id=4)
                        preCloseCustomerEMI.TransactionId = Transaction_Id
                        preCloseCustomerEMI.emiSubmittedBy = User.objects.get(id=request.user.id)
                        preCloseCustomerEMI.EmiPaymentMode = DisbursementMode.objects.get(
                        id=4
                        )
                        preCloseCustomerEMI.save()
                    messages.success(request, "Loan is disbursed successfully With Last loan Preclosed")
                    return redirect('successRenderScreen')
                else:
                    messages.success(request, f"Loan is disbursed successfully{pre_closure_loanID}")
                    return redirect('successRenderScreen')
            except:
                messages.success(request, f"Loan is disbursed successfully{pre_closure_loanID}")
                return redirect('successRenderScreen')
...
def addCollections(request, id):
    principalArrearAmount = 0
    intrestArrearAmount = 0
    days = 0
    emiForUpdate = None
    emiPayMode = None
    nextEmi = None
    ...
    allCSO = userWithRole.objects.filter(branchName__id = emiCustomerData.BranchName.branchName.id).exclude(userName__is_superuser = True)
    ...
    try:
        noOfDays = loanEMI.objects.filter(
        loanId__loanId=emiCustomerData.loandata.loanId ,emiStatus__id = 3
        ).first() 
        if noOfDays != None:
            days = abs((noOfDays.installmentDate.date() - datetime.today().date()).days) + 1
    except loanEMI.DoesNotExist:
        days = 0
    notPaidEmiList = loanEMI.objects.filter(
        loanId__loanId=emiCustomerData.loandata.loanId ,emiStatus__id = 3
    )
    ...
    paidPreCloseAmount = loanEMI.objects.filter(
        loanId__loanId=emiCustomerData.loandata.loanId, emiStatus__id=2
    )
    
    if len(paidPreCloseAmount) == 0:
        for notPaidEmi in emiList:
            if notPaidEmi.emiStatus.id == 3:
                principalArrear = notPaidEmi
                amount = emiCustomerData.LoanAmount + principalArrear.interestAmoun
    else:
        paidPreclosureAmount = loanEMI.objects.filter(
        loanId__loanId=emiCustomerData.loandata.loanId, emiStatus__id=2
        ).last()
        amount = preclosureAmount.interestAmoun + paidPreclosureAmount.outstandingPrincipal
    if preclosureAmount:
        nextEmi = loanEMI.objects.get(id=((preclosureAmount.id) + 1))
        collectionSequanceNo = len(emiList) + 1
    for notPaidEmi in notPaidEmiList:
        principalArrearAmount = principalArrearAmount+ notPaidEmi.principleAmount
        intrestArrearAmount = intrestArrearAmount + notPaidEmi.interestAmoun     
    context = {
        "emiCustomerData": emiCustomerData,
        "collectionSequanceNo": collectionSequanceNo,
        "preclosureAmount": preclosureAmount,
        "intrestArrearAmount": intrestArrearAmount,
        "allCSO":allCSO,
        "amount":amount,
        "principalArrear": principalArrearAmount,
        "nextEmi": nextEmi,
        "days": days ,
...
def addCollectionForm(request, loanID):
    i = 0
    emiList = loanEMI.objects.filter(loanId__loanId=loanID).exclude(emiStatus__id=1)
    ...
    if request.method == "POST":
        ...
        Collection_Pay_Type = request.POST.get("CollectionPayType")
        number_of_emi = request.POST.get("numberOfEMI")
        emi_submitted_by = userWithRole.objects.get(id = CollectedBy)
        Transaction_Id = (
            1001
            if loanEMI.objects.filter(loanId__loanId=loanID, emiStatus__id=2).count()
            == 0
            else loanEMI.objects.filter(
                loanId__loanId=loanID, emiStatus__id=2
            ).aggregate(max=Max("TransactionId"))["max"]
            + 1
        )
    if (
        ...
        if paidEMI.PaymentDate is None and int(Collection_Pay_Type) == 4:
            for update_emi_installment in exclude_paid_emi:
                update_emi_installment.emiStatus = emiStatus.objects.get(id=2)
                update_emi_installment.TransactionId = Transaction_Id
                update_emi_installment.emiSubmittedBy = emi_submitted_by.userName 
            ...
            and paidEMI.emiStatus.id == 2
        ):
            message = "You can submit only one installment on same day"
        elif int(Collection_Pay_Type) == 4:
            for update_emi_installment in exclude_paid_emi:
                update_emi_installment.emiStatus = emiStatus.objects.get(id=2)
                update_emi_installment.TransactionId = Transaction_Id
                update_emi_installment.emiSubmittedBy = emi_submitted_by.userName 
        ...
        if int(Collection_Pay_Type) == 6:
            for update_emi_installment in exclude_paid_emi:
                ...
                message = "EMI Preclosed successfully"
                update_emi_installment.save()
...
def emiAmount(request):
    if request.method == "GET":
        ...
        emiInstallmentAmount = loanEMI.objects.filter(loanId__loanId=loanid).exclude(emiStatus__id=2)
        for emiAmount in emiInstallmentAmount:
            amount = amount + emiAmount.installmentAmount
            i += 1
            if i == no_of_emi:
                break
    return render(request, "no_of_emi.html", {"amount": amount,"id":emiInstallmentAmount})

@login_required(login_url="/")
def Bank_Transaction_Deposit(request):
    Branch = branch.objects.all()
    Bank = BankName.objects.filter(id = 133)
    if request.method == "POST":
        branchName = request.POST.get("branchName")
        ...
        # return render(request, "successRenderScreen.html")
        return redirect('successRenderScreen')
    context = {"Branch": Branch, "Banks": Bank}
...
def customerAccountStatement(request, id):
    ...
    allPaidEmi = loanEMI.objects.filter(loanId__loanId = customer_data.loandata.loanId,emiStatus__id__in = [2,4])
    allEmi = loanEMI.objects.filter(loanId__loanId = customer_data.loandata.loanId)
    lastPaidEmai = loanEMI.objects.filter(loanId__loanId = customer_data.loandata.loanId,emiStatus__id = 2).last()
...
def data_update(request):
    ...
    Data.status = form_STATUS.objects.get(id=1)
    # for data in Data:
    #     data.status= form_STATUS.objects.get(id = 1)
    Data.save()
...
auth_token = "d1ebe0c317a632e5eeb09466e908ad14"
    ...
    try:
        if request.method == "GET":
            ...
            message = client.messages.create(
                ...
                from_="+12513136594",  # your Twilio phone number
                body=f"Your OTP Testing for Parishkar Microcare Foundation LMS : {otp}",
            )
            data = {
...
def generate_pdf(request):
    ...
    if request.method == "POST":
        ...
        openingBalanceAmount = Account_Transaction.objects.filter(TransDate = To_Date)
        if openingBalanceAmount:
            for openingBalances in openingBalanceAmount:
                openingBalance = openingBalances.AccountBalance
        else:
...
def dataUpdate(request):
    data = loanEMI.objects.get(id = 485)
    data.emiSubmittedBy = None
    data.EmiPaymentMode = None
