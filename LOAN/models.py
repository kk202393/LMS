...
class center(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.center_Id:
            ...
            maxCenterID = 1 if center.objects.count() == 0 else center.objects.aggregate(max=Max("id"))["max"] + 1
...
class approved_customer_kyc(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(User, on_delete=models.PROTECT)
    status = models.ForeignKey(form_STATUS, default=2, on_delete=models.PROTECT)
...
class rejected_customer_kyc(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(User, on_delete=models.PROTECT)
    status = models.ForeignKey(form_STATUS, default=3, on_delete=models.PROTECT)
    # Application_No = models.IntegerField(unique=True, null=True)
    loandata = models.ForeignKey(
        Loan_Application_Details, unique=True, on_delete=models.CASCADE
    )
...
class loanEMI(models.Model):
    ...
    loanId = models.ForeignKey(Loan_Application_Details, on_delete=models.CASCADE)
    emiStatus = models.ForeignKey(emiStatus, on_delete=models.PROTECT, default=1)
...
class disbursement_customer_kyc(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(User, on_delete=models.PROTECT)
    status = models.ForeignKey(form_STATUS, default=4, on_delete=models.PROTECT)
    ...
    def intrestCalculation(self):
        totalIntrestAmount = []
        totalOSIntrestAmount = []
        totalIntrest = loanEMI.objects.filter(loanId__loanId = self.loandata.loanId)
        totalOSIntrest = loanEMI.objects.filter(loanId__loanId = self.loandata.loanId,emiStatus__id = 1)
        for intrest in totalIntrest:
            totalIntrestAmount.append(intrest.interestAmoun)
        for intrestOS in totalOSIntrest:
            totalOSIntrestAmount.append(intrestOS.interestAmoun)
        return  {"totalIntrestAmount":sum(totalIntrestAmount), "totalOSIntrestAmount":sum(totalOSIntrestAmount),"OSInstallments":len(totalOSIntrest)}
    
    def lastPaidEmiData(self):
        return loanEMI.objects.filter(loanId__loanId = self.loandata.loanId,emiStatus__id = 2).last()
    ...
    def Pre_Closure_Amount(self):
        ...
        OSInstallments = []
        principleArrear = []
        preClosePaidAmount = 0
        intrestArrear = []
        installmentArrear=[]
        totalEMIIntrest = []
        totalIntrest = loanEMI.objects.filter(loanId__loanId=self.loandata.loanId)
        arrearAmount = loanEMI.objects.filter(loanId__loanId=self.loandata.loanId , emiStatus__id=3)
        paidEMI = loanEMI.objects.filter(loanId__loanId=self.loandata.loanId , emiStatus__id=2)
        totalArrear = loanEMI.objects.filter(loanId__customerId=self.loandata.loanId,emiStatus__id__in = [1,3])
        ...
        lastPaidEmai = loanEMI.objects.filter(loanId__loanId = self.loandata.loanId,emiStatus__id = 2).last()
        if lastPaidEmai is None:
             preClosePaidAmount = sum(intrestArrear) + self.LoanAmount
        else:
            lastEmiIntrest = loanEMI.objects.filter(
            loanId__loanId=self.CustomerEMI.loanId.loanId, emiStatus__id=1
        ).first()
            preClosePaidAmount = sum(intrestArrear) + lastPaidEmai.outstandingPrincipal + lastEmiIntrest.interestAmoun
        return {"totalPreClosureAmount":sum(totalPreClosureAmount), "OSPrinciple":sum(OSPrinciple),"OSInterest":sum(OSInterest),"OSInstallments":(len(totalEMIIntrest)-len(paidEMI)),"principleArrear":sum(principleArrear),"intrestArrear":sum(intrestArrear),"installmentArrear":preClosePaidAmount ,"totalEMIIntrest":sum(totalEMIIntrest)}
    ...
    def paidEMI(self):
        ...
        return {"collectedEMI": collectedEMI, "days": abs(days)}
